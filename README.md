...
Leave the first switch activated if you want to save them in the same folder, if you prefer to choose a specific folder, turn it off. You can also generate a tiny icon by activating the second switch. Finally, click on `Convert` button, wait for the interface to restart because this indicates that it has finished and your icons are ready.
...
- **Format:** Allows you to choose what sizes will be included **within the icon**. Click on `Format` to display a menu with two options (check the box); **Multiple** generates a high quality icon that include all necessary sizes such as 16, 32, 48, 64, 128 and 256 pixels, although in the dimensions or details it will show only 16px. On the other hand, **256 px** generates an icon of a single size but these look pixelated when displayed small.
     >Note: Note: If the "Generate tiny icon" switch is activated, a small icon will be generated regardless of the setting chosen in "Format".
